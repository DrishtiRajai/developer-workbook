...
function createTodo() {
  ...
  if (
    (is_btnActive && checkbox.checked === false) ||
    (is_btnCompleted && checkbox.checked === false)
  ) {
    saveTodo(todos, false, getActive);
  } else if (
    (is_btnCompleted && checkbox.checked === true) ||
    (is_btnActive && checkbox.checked === true)
  ) {
...
function todoList(filteredItems, filtered) {
  ...
  filteredItems.forEach((todo) => {
    ...
    newItem.className = `list-items todo ${
    ...
    newItem.innerHTML = `<div class="round-checkbox">
      ...
      todo.completed === true ? "checked" : ""
            ...
            <a href="javascript:void(0)" role="button" class="btn-delete" data-delete-id="${
              todo.id
            }"></a>`;
  ...
  total_items.innerHTML = `${items_left.length} item${
    items_left.length !== 1 ? "s" : ""
  } left`;
}
  ...
  element.forEach((element) => {
    ...
    if (completedEl) {
      ...
      listfilter.forEach((lf) => {
        const btnAll =
          lf.classList.contains("btnAll") && lf.classList.contains("active");
        const btnActive =
          lf.classList.contains("btnActive") && lf.classList.contains("active");
        const btnCompleted =
          lf.classList.contains("btnCompleted") &&
          lf.classList.contains("active");
        if (btnActive) {
          getActive();
        }
        if (btnCompleted) {
